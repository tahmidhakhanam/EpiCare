    ...
    <body>
        ...
        <div class="container-main">
            ...
            <?php
                ...
                $contactName = filter_has_var(INPUT_POST, 'contactName') ? $_POST['contactName']: null;
                $contactEmail = filter_has_var(INPUT_POST, 'email') ? $_POST['email']: null;
                $contactPhone = filter_has_var(INPUT_POST, 'phone') ? $_POST['phone']: null;
                ...
                //sanitise variables
                $contactName = filter_var($contactName, FILTER_SANITIZE_STRING);
                $contactEmail = filter_var($contactEmail, FILTER_SANITIZE_STRING);
                $contactPhone = filter_var($contactPhone, FILTER_SANITIZE_STRING);

                //validate data
                if (empty($contactName)) {

                    //Output error message
                    echo "<h1 class='profile-error text-center'>Contact is required field.</h1>\n";
                    echo "<p class='profile-error text-center'>Click<a href='myinfo.php'> here</a> to return to my Info </p>";
                } else if (empty($contactEmail)) {

                    //Output error message
                    echo "<h1 class='profile-error text-center'>Email is required field.</h1>\n";
                    echo "<p class='profile-error text-center'>Click<a href='myinfo.php'> here</a> to return to my Info </p>";
                } else if (!preg_match('/^\d{11}$/', $contactPhone)) {

                    //Output error message
                    echo "<h1 class='profile-error text-center'>Phine numbers must only contain 0-9 and have 11 characters</h1>\n";
                    echo "<p class='profile-error text-center'>Click<a href='myinfo.php'> here</a> to return to my Info </p>";
                } else {
                    try {
                        ...
                        if (isset($_SESSION['user_id'])) {
                            ...
                            //insertsql
                            $insertSQL = "INSERT INTO epicare_contacts (user_id, name, email, phone) VALUES (:user_id, :name, :email, :phone)";

                            //Execute query
                            $stmt = $dbConn->prepare($insertSQL);
                            $stmt->execute(array(':user_id' => $user_id, ':name' => $contactName, ':email' => $contactEmail, ':phone'=> $contactPhone));
                        ...
                        //Output error message
                        echo "<h1 class='profile-error text-center'>There was a problem adding your contact..</h1>\n";
